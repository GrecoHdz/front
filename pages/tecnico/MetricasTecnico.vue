  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div v-if="isLoading" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-xl">
        <div class="animate-spin rounded-full h-10 w-10 border-3 border-blue-500 border-t-transparent mx-auto mb-4"></div>
        <p class="text-gray-700 dark:text-gray-300 text-sm">Cargando métricas...</p>
      </div>
    </div>
      ...
      <div class="max-w-2xl mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h1 class="text-lg font-black text-gray-900 dark:text-white">Mis Métricas</h1>
    ...
    <!-- Content Container -->
    <div class="max-w-2xl mx-auto bg-gray-50 dark:bg-gray-900 min-h-screen pb-20">
      
      <!-- Balance y Ganancias -->
      <section class="px-4 py-4">
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl p-6 text-white mb-4">
          <div class="flex items-center justify-between mb-4">
            <div>
              <p class="text-green-100 text-sm font-medium">Balance Disponible</p>
              <p class="text-3xl font-black">L. {{ formatCurrency(balance.balanceDisponible) }}</p>
            </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p class="text-xl font-bold">{{ balance.ultimoRetiro !== null ? `L. ${formatCurrency(parseFloat(balance.ultimoRetiro))}` : 'N/A' }}</p>
            </div>
            <div>
              <p class="text-green-100 text-xs">Último Ingreso</p>
              <p class="text-xl font-bold">{{ balance.ultimoIngreso !== null ? `L. ${formatCurrency(parseFloat(balance.ultimoIngreso))}` : 'N/A' }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="px-4 mb-6">
        <h2 class="text-lg font-black text-gray-900 dark:text-white mb-4">Estadísticas Generales</h2>
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <p class="text-2xl font-black text-gray-900 dark:text-white">{{ stats.totalServices }}</p>
                <p class="text-xs text-gray-600 dark:text-gray-400 font-bold">Servicios Totales</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700">
            <div class="flex items-center space-x-3">
              ...
              <div>
                ...
                <p class="text-xs text-gray-600 dark:text-gray-400 font-bold">Calificación</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <p class="text-2xl font-black text-gray-900 dark:text-white">{{ stats.last3Months }}</p>
                <p class="text-xs text-gray-600 dark:text-gray-400 font-bold">Últimos 3 Meses</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <p class="text-2xl font-black text-gray-900 dark:text-white">{{ stats.thisMonth }}</p>
                <p class="text-xs text-gray-600 dark:text-gray-400 font-bold">Este Mes</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="px-4 mb-4">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-black text-gray-900 dark:text-white">Análisis de Datos</h2>
          <select v-model="selectedChart" 
                  class="px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
...
<section class="px-4 mb-6">
  ...
  <div class="space-y-2">
    ...
    <template v-else>
      ...
      <section class="px-4 mb-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-lg font-black text-gray-900 dark:text-white">Calificaciones Recientes</h2>
          <span class="text-sm text-gray-600 dark:text-gray-400">{{ reviews.length }} calificaciones</span>
        </div>
        
        <!-- Mensaje cuando no hay calificaciones -->
        <div v-if="reviews.length === 0" class="bg-white dark:bg-gray-800 rounded-xl p-8 text-center border-2 border-dashed border-gray-200 dark:border-gray-700">
          ...
          <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">Sin calificaciones aún</h3>
        ...
        <template v-else>
          <div class="space-y-3">
            <div v-for="review in (showAllReviews ? reviews : reviews.slice(0, 3))" :key="review.id || `${review.nombre_cliente}-${review.fecha}`"
              ...
              <div class="flex items-start justify-between mb-3">
                <div class="flex items-center space-x-3">
                  <div class="w-10 h-10 bg-gradient-to-r from-purple-400 to-pink-500 rounded-full flex items-center justify-center">
                    <span class="text-white text-sm font-bold">{{ getInitials(review.nombre_cliente) }}</span>
                  </div>
                  <div>
                    <p class="font-bold text-gray-900 dark:text-white text-sm">{{ review.nombre_cliente }}</p>
              ...
              <p class="text-gray-700 dark:text-gray-300 text-sm mb-3">{{ review.comentario }}</p>
              <div class="flex items-center space-x-2">
          ...
          <button v-if="pagination.hasMore"
                  ...
                  :disabled="isLoading"
                  class="w-full mt-3 py-2 text-blue-600 dark:text-blue-400 text-sm font-medium hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div v-if="showWithdrawModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm max-h-[85vh] overflow-y-auto relative z-10">
        <!-- Modal Header -->
        <div class="sticky top-0 bg-white dark:bg-gray-800 p-4 border-b border-gray-200 dark:border-gray-700 rounded-t-2xl">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h3 class="text-lg font-black text-gray-900 dark:text-white">Retirar Dinero</h3>
        ...
        <div class="p-4">
          <!-- Monto a retirar -->
          <div class="mb-6">
            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Monto a retirar</label>
            <div class="relative">
              <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400">L.</span>
              <input v-model.number="withdrawForm.amount" 
                     type="number" 
                     step="0.01" 
                     :max="balance.balanceDisponible"
                     min="1"
                     class="w-full pl-8 pr-4 py-3 text-base border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white" 
                     placeholder="0.00">
            </div>
          ...
          <!-- Datos Bancarios -->
          <div class="mb-6">
            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Datos Bancarios</label>
            <textarea v-model="withdrawForm.bankDetails" 
                      rows="4" 
                      class="w-full px-3 py-3 text-base border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white" 
          ...
          <button @click="processWithdraw" 
                  ...
                  class="w-full py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold rounded-xl hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all">
...
const stats = reactive({
  totalServices: 0,
  rating: 4.8, // Mantenemos el rating estático por ahora
  last3Months: 0,
  thisMonth: 0
})
...
const getWithdrawalNumber = (withdrawal) => {
  ...
  // Ordenar todos los retiros por fecha
  allWithdrawals.sort((a, b) => new Date(a.fecha) - new Date(b.fecha))
  
  // Encontrar el índice del retiro actual y sumar 1 (para que empiece en 1)
...
const loadReviews = async (loadMore = false) => {
  try {
    ...
    // Si no es una carga de más reseñas, reiniciamos la paginación
    if (!loadMore) {
    ...
    const newReviews = response.data.map(review => ({
      id: `${review.nombre_cliente}-${review.fecha}`, // ID único basado en cliente y fecha
    ...
    // Si es una carga de más reseñas, las agregamos al final
    if (loadMore) {
...
const createChart = () => {
  ...
  switch (selectedChart.value) {
          ...
          plugins: {
            ...
            datalabels: {
              ...
              color: function(context) {
                // Cambiar el color del texto según el color de la barra
              ...
              textStrokeWidth: 1,
              // Asegurar que el texto se muestre dentro de la barra
              clamp: true,
              clip: false,
              // Ajustar la posición vertical
              offset: 0,
              // Solo mostrar si la barra es lo suficientemente alta
              display: function(context) {
                return context.dataset.data[context.dataIndex] > 0; // Solo mostrar si el valor es mayor a 0
              }
            }
          },
          scales: {
    ...
    case 'serviceTypes':
      ...
      // Calcular porcentajes para mostrar en los segmentos
          ...
          plugins: {
            legend: {
              ...
              labels: {
                ...
                // Sobrescribir el renderizado de la leyenda
...
const loadEstadisticasGenerales = async () => {
  try {
    ...
    console.log('Valores numéricos:', {
      balanceDisponible: data.balanceDisponible,
      ultimoRetiro: data.ultimoRetiro,
      ultimoIngreso: data.ultimoIngreso,
      tipoBalance: typeof data.balanceDisponible,
      tipoRetiro: typeof data.ultimoRetiro,
      tipoIngreso: typeof data.ultimoIngreso
    });
    
    // Función para asegurar que un valor sea un número válido o 0
    const safeNumber = (value) => {
      const num = Number(value);
      return isNaN(num) ? 0 : num;
    };
    
    // Actualizar balance con los datos del servidor
    ...
    console.log('Balance actualizado:', balance.value);
...
const loadServicesByType = async () => {
  try {
    ...
    // Update chart if it's the currently selected one
...
const loadMonthlyIncomes = async () => {
  try {
    ...
    // Si el gráfico actual es el de ingresos, actualizarlo
...
const loadMonthlyServices = async () => {
  try {
    ...
    // Si el gráfico actual es el de servicios, actualizarlo
...
onMounted(async () => {
  ...
  await Promise.all([
    ...
    loadMovements(),
    loadReviews() // Agregar carga de calificaciones
  ])
