<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <LoadingSpinner 
      :loading="isLoading" 
      :message="$t('technician.referral.messages.loading_program')"
    />

    <!-- Contenido principal -->
    <div v-if="!isLoading">
      <HeadersHeaderReferidos />
      ...
      <div class="max-w-2xl mx-auto bg-gray-50 dark:bg-gray-900 min-h-screen relative">
        ...
        <div>
          ...
          <main>
            ...
            <section class="px-4 py-4">
              <div class="bg-gradient-to-br from-purple-600 via-pink-600 to-red-500 rounded-2xl p-5 text-white relative overflow-hidden">
                ...
                <div class="relative text-center">
                  ...
                  <h1 class="text-2xl font-black mb-2">{{ $t('technician.referral.hero_title') }}</h1>
                  <p class="text-white/90 font-medium mb-5 text-sm">
                    {{ $t('technician.referral.hero_subtitle', { percentage: referralReward }) }}
                  </p>
                  
                  <!-- Stats -->
                  <div class="grid grid-cols-3 gap-3"> 
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                      <div class="text-xl font-black">L. {{ formatCurrency(stats.totalEarnings) }}</div>
                      <div class="text-xs text-white/80">{{ $t('technician.referral.stats.total_earned') }}</div>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                      <div class="text-xl font-black">L. {{ formatCurrency(stats.retirado) }}</div>
                      <div class="text-xs text-white/80">{{ $t('technician.referral.stats.total_withdrawn') }}</div>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                      <div class="text-xl font-black">L. {{ formatCurrency(stats.availableBalance) }}</div>
                      <div class="text-xs text-white/80">{{ $t('technician.referral.stats.available') }}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <section class="px-4 mb-4">
              <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-lg border border-gray-100 dark:border-gray-700">
                <div class="text-center mb-4">
                  ...
                  <h2 class="text-xl font-black text-gray-900 dark:text-white mb-2">{{ $t('technician.referral.title') }}</h2>
                  <p class="text-gray-600 dark:text-gray-400 text-sm">{{ $t('technician.referral.subtitle') }}</p>
                </div> 

                <!-- Enlace de referido -->
                <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl mb-4">
                  <p class="text-xs font-bold text-gray-600 dark:text-gray-400 mb-2">{{ $t('technician.referral.invite_link') }}</p>
                ...
                <div class="grid grid-cols-2 gap-2">
                  ...
                  >
                    <span class="text-lg">🔗</span>
                    <span class="font-bold text-sm">{{ $t('technician.referral.share') }}</span>
                  </button>
                </div>
              </div>
            </section>
            ...
            <!-- Cómo Funciona -->
            <section class="px-4 mb-4">
              <h2 class="text-lg font-black text-gray-900 dark:text-white mb-3">{{ $t('technician.referral.how_it_works') }}</h2>
              <div class="space-y-2">
            ...
            <section class="px-4 mb-6">
              <h2 class="text-lg font-black text-gray-900 dark:text-white mb-3">{{ $t('technician.referral.history') }}</h2>
              <div class="flex items-center justify-between mb-4">
                <div class="flex space-x-2">
                  ...
                  >
                    {{ $t('technician.referral.withdrawals') }}
                  </button>
                </div>
              ...
              <div class="space-y-2">
                ...
                <template v-if="activeTab === 'ingresos'">
                  ...
                  <div v-if="isLoadingMovements" class="text-center py-8">
                    ...
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ $t('technician.referral.messages.loading_incomes') }}</p>
                  </div>
                  ...
                  <div v-else-if="filteredEarnings.length === 0" class="bg-white dark:bg-gray-800 rounded-xl p-6 text-center border-2 border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">{{ $t('technician.referral.messages.no_incomes') }}</h3>
                  ...
                  <template v-else>
                      ...
                      <div class="flex items-center justify-between">
                          ...
                          <div>
                            <p class="font-bold text-gray-900 dark:text-white text-sm">
                              <template v-if="earning.descripcion && earning.descripcion.includes('Comisión por referido')">
                                {{ earning.descripcion.includes('(Paquete)') ? $t('technician.referral.labels.package') : $t('technician.referral.labels.service') }} {{ $t('technician.referral.labels.acquired') }} {{ (earning.descripcion.split(' - ')[1] || 'Usuario').split(' ').slice(0, 2).join(' ') }}
                              </template>
                              <template v-else>
                                {{ earning.descripcion || $t('technician.referral.labels.referral_earning') }}
                              </template>
                            </p>
                        ...
                        <div class="text-right">
                          ...
                          <div class="relative group">
                            <p class="text-xs flex items-center justify-end" :class="getStatusColor(earning.estado)">
                              <template v-if="earning.estado.toLowerCase() === 'rechazado'">
                                <span class="inline-flex items-center relative">
                                  <svg class="w-3.5 h-3.5 mr-1 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                  </svg>
                                  {{ earning.estado }}
                                  <span class="absolute z-20 invisible group-hover:visible w-48 bg-gray-800 text-white text-xs rounded p-2 -left-1/2 -top-20 opacity-0 group-hover:opacity-100 transition-opacity duration-200 transform -translate-x-1/2">
                                    {{ $t('technician.referral.messages.rejected_reason') }}
                    ...
                    <div v-if="hasMoreMovements" class="text-center mt-4">
                      ...
                      >
                        ...
                        <span v-else>
                          {{ $t('technician.referral.labels.view_more_incomes', { n: totalMovements - filteredEarnings.length }) }}
                        </span>
                      </button>
                    </div>
                      ...
                      <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2"> 
                          <span class="text-sm font-medium text-green-700 dark:text-green-300">
                            {{ $t('technician.referral.labels.total_month', { month: movementSummary.mes }) }}
                          </span>
                        </div>
                ...
                <template v-else>
                  ...
                  <div v-if="isLoadingMovements" class="text-center py-8">
                    <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto"></div>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ $t('technician.referral.messages.loading_withdrawals') }}</p>
                  </div>
                  ...
                  <div v-else-if="filteredWithdrawals.length === 0" class="bg-white dark:bg-gray-800 rounded-xl p-6 text-center border-2 border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">{{ $t('technician.referral.messages.no_withdrawals') }}</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <template v-else>
                        ...
                        <div class="flex items-center space-x-3">
                          ...
                          <div>
                            <p class="font-medium text-gray-900 dark:text-white">
                              {{ `${$t('technician.referral.labels.fund_withdrawal')} #${withdrawalCounter - filteredWithdrawals.findIndex(w => w.id === withdrawal.id)}` }}
                            </p>
                            <p class="text-xs text-gray-600 dark:text-gray-400">
                    ...
                    <div v-if="hasMoreMovements" class="text-center mt-4">
                      ...
                      >
                        ...
                        <span v-else>
                          {{ $t('technician.referral.labels.view_more_withdrawals', { n: totalMovements - filteredWithdrawals.length }) }}
                        </span>
                      </button>
                    </div>
              ...
              <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-lg border border-gray-100 dark:border-gray-700 relative">
                <div class="relative">
                  <div class="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <h2 class="text-xl font-black">{{ $t('technician.referral.withdraw.title') }}</h2>
                      <p class="text-white/90 text-sm">
                        {{ $t('technician.referral.withdraw.available', { amount: formatCurrency(stats?.availableBalance || 0) }) }}
                        <span v-if="withdrawalPercentage > 0" class="block text-xs opacity-80">
                          {{ $t('technician.referral.withdraw.withdrawable_limit', { percentage: withdrawalPercentage, amount: formatCurrency(maxWithdrawableAmount || 0) }) }}
                        </span>
                      </p>
                    </div>
                  </div>
                    ...
                    >
                      <template v-if="stats?.availableBalance > 0">
                        <template v-if="stats.availableBalance >= minWithdrawAmount">
                          {{ $t('technician.referral.withdraw.btn') }}
                        </template>
                        <template v-else>
                          {{ $t('technician.referral.withdraw.min_amount', { amount: formatCurrency(minWithdrawAmount) }) }}
                        </template>
                      </template>
                      <template v-else>
                        {{ $t('technician.referral.withdraw.no_balance') }}
                      </template>
                    </button>
                  </div>
                </div>
            ...
            <section class="px-4 mb-4">
              <div class="flex items-center justify-between mb-3">
                <h2 class="text-lg font-black text-gray-900 dark:text-white">{{ $t('technician.referral.your_referrals') }}</h2>
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $t('technician.referral.people_count', { n: stats.totalReferrals || 0 }) }}</span>
              </div>
              
              <!-- Loading -->
              <div v-if="isLoadingReferrals" class="text-center py-8">
                <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto"></div>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ $t('technician.referral.messages.loading_referrals') }}</p>
              </div>
              ...
              <div v-else-if="!referralHistory || referralHistory.length === 0" class="text-center py-6">
                ...
                <h3 class="text-base font-black text-gray-900 dark:text-white mb-2">{{ $t('technician.referral.messages.no_referrals') }}</h3>
                <p class="text-gray-600 dark:text-gray-400 mb-3 text-sm">{{ $t('technician.referral.messages.no_referrals_desc') }}</p>
              </div>
                ...
                <div v-if="referralsPagination.totalItems > 0" class="mt-3 bg-white dark:bg-gray-800 p-2 rounded-lg">
                  <div class="flex items-center justify-between">
                    <div class="text-xs text-gray-500 dark:text-gray-400">
                      {{ $t('technician.referral.pagination', { current: referralHistory.length, total: referralsPagination.totalItems }) }}
                    </div>
                    <div class="flex items-center space-x-2">
                      <button 
                      ...
                      <span class="px-2 text-xs font-medium text-gray-700 dark:text-gray-300">
                        {{ $t('technician.dashboard.pagination_info', { current: referralsPagination.currentPage, total: referralsPagination.totalPages }) }}
                      </span>
            ...
            <section class="px-4 mb-4">
              <div class="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-3 border border-amber-200 dark:border-amber-800">
                <div class="flex items-start space-x-2">
                  ...
                  <div>
                    <h3 class="text-sm font-bold text-amber-800 dark:text-amber-200 mb-2">{{ $t('technician.referral.terms_title') }}</h3>
            ...
            <div class="sticky top-0 bg-white dark:bg-gray-800 p-3 border-b border-gray-200 dark:border-gray-700 rounded-t-xl z-10">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  ...
                  <div>
                    <h3 class="text-base font-black text-gray-900 dark:text-white">{{ $t('technician.referral.withdraw.title') }}</h3>
                    <p class="text-xs text-gray-600 dark:text-gray-400">{{ $t('technician.referral.withdraw.available', { amount: formatCurrency(stats.availableBalance) }) }}</p>
                  </div>
                </div>
              ...
              <form @submit.prevent="processWithdraw" class="space-y-4">
                ...
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                    {{ $t('technician.referral.withdraw.form.amount_label', { percentage: withdrawalPercentage }) }}
                  </label>
                  <div class="relative">
                ...
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                    {{ $t('technician.referral.withdraw.form.credit_label', { percentage: 100 - withdrawalPercentage }) }}
                  </label>
                  <div class="relative">
                    <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400 font-bold">L.</span>
                    <input
                ...
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                    {{ $t('technician.referral.withdraw.form.bank_details_label') }}
                  </label>
                  <textarea
                    ...
                    :placeholder="$t('technician.referral.withdraw.form.bank_details_placeholder')"
                    required
                  ></textarea>
                </div>
                ...
                >
                  <span v-if="!isProcessingWithdraw">
                    {{ $t('technician.referral.withdraw.form.submit_btn') }}
                  </span>
                  <span v-else class="flex items-center justify-center">
                    ...
                    {{ $t('technician.referral.withdraw.form.processing') }}
                  </span>
                </button>
              </form>
            </div>
...
// SEO and Meta
useHead({
  title: t('technician.referral.title'),
  meta: [
    { name: 'description', content: t('technician.referral.hero_subtitle', { percentage: 50 }) },
...
const howItWorksSteps = computed(() => [
  {
    title: t('technician.referral.steps.share_code.title'),
    description: t('technician.referral.steps.share_code.desc'),
    icon: '📱'
  },
  {
    title: t('technician.referral.steps.registration.title'),
    description: t('technician.referral.steps.registration.desc'),
    icon: '✍️'
  },
  {
    title: t('technician.referral.steps.hiring.title'),
    description: t('technician.referral.steps.hiring.desc'),
    icon: '🏠'
  },
  {
    title: t('technician.referral.steps.reward.title'),
    description: t('technician.referral.steps.reward.desc'),
    icon: '💰'
  }
])

// Términos del programa
const referralTerms = computed(() => {
  return t('technician.referral.terms', { returnObjects: true }) || []
})
...
const formatCurrency = (amount) => {
  const { locale } = useI18n()
  return parseFloat(amount).toLocaleString(locale.value === 'en' ? 'en-US' : 'es-HN', {
...
const getNoEarningsMessage = () => {
  ...
  if (selectedYear === currentYear.toString()) {
    return t('technician.metrics.messages.no_earnings_month', { month: monthName, year: selectedYear })
  } else {
    return t('technician.metrics.messages.no_earnings_generic')
  }
}

const getNoWithdrawalsMessage = () => {
  const currentYear = new Date().getFullYear()
  const [selectedYear, selectedMonthNum] = selectedWithdrawMonth.value.split('-')
  const monthName = t(`common.months.${parseInt(selectedMonthNum) - 1}`)
  
  if (selectedYear === currentYear.toString()) {
    return t('technician.metrics.messages.no_withdrawals_month', { month: monthName, year: selectedYear })
  } else {
    return t('technician.metrics.messages.no_withdrawals_generic')
  }
}
...
const loadReferrals = async (page = 1) => {
  ...
  try {
    ...
    showError(t('technician.referral.messages.loading_error'))
...
const loadMovements = async (tipo = 'ingreso_referido', month = null, loadMore = false) => {
  ...
  try {
    ...
    showError(t('technician.referral.messages.history_error'))
    if (tipo === 'ingreso_referido') {
      movementsHistory.value = []
    } else {
      withdrawalsHistory.value = []
...
const shareWhatsApp = () => {
  const message = t('technician.referral.messages.share_whatsapp_message', { link: referralLink.value })
...
const shareGeneral = async () => {
  const shareData = {
    ...
    text: t('technician.referral.messages.share_general_text', { code: userReferralCode.value }),
...
const processWithdraw = async () => {
  ...
  try {  
    ...
    if (!userCookie.value || !userCookie.value.id_usuario) {
      showError(t('technician.dashboard.messages.user_error'));
      return;
    }
    ...
    if (montoRetiro < minWithdrawAmount) {
      showError(t('technician.referral.withdraw.min_amount_error', { min: formatCurrency(minWithdrawAmount), bal: formatCurrency(montoRetiro) }));
    ...
    if (montoMaximoRetiro <= 0) {
      showError(t('technician.referral.withdraw.no_balance_error'));
      isProcessingWithdraw.value = false;
      return;
    }
    ...
    if (response && response.success === true) {
      ...
      if (montoCredito > 0) {
        try { 
          ...
          showSuccess(t('technician.referral.messages.withdraw_success'), t('technician.referral.messages.withdraw_pending_desc'));
        } catch (creditError) {
    ...
    showError(t('common.error_unexpected'));
  } finally {
    isProcessingWithdraw.value = false;
  }
}
