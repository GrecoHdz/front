                  ...
                  <div class="flex flex-col w-full h-full"> 
                     ...
                     <div v-if="paquete.fecha_compra || paquete.fecha_uso" class="mt-2 pt-1 border-t border-gray-100 dark:border-gray-700/50 space-y-0.5">
                        ...
                        <div v-if="paquete.fecha_uso" class="flex items-center gap-1.5 text-[8px] text-blue-500">
                           <span class="font-black uppercase tracking-tighter">{{ $t('dashboard_client.in_use') }}:</span>
...
const laneHome = computed(() => {
   ...
   return homePackagesSource.value.slice(0, 5)
})

const lane1 = computed(() => {
   if (searchQuery.value) return []
   return otherPackagesSource.value.slice(0, 4)
})

const lane2 = computed(() => {
   if (searchQuery.value) return []
   return otherPackagesSource.value.slice(4, 8)
})
...
const verDatosCuenta = (acc) => {
   ...
   selectedAccountObject.value = acc
...
const formatDate = (dateString) => {
  ...
  return date.toLocaleDateString(locale.value === 'es' ? 'es-MX' : 'en-US', {
    day: '2-digit',
    month: locale.value === 'es' ? 'short' : '2-digit',
    year: 'numeric',
    hour: '2-digit',
    minute: '2-digit',
    hour12: true
...
const getEstadoPaquete = (id) => {
   ...
   if (ps.some(p => p.estado === 'activo')) return t('marketplace.status.adquired')
...
const confirmarCanjeo = async () => {
   try {
      ...
      if(res.success) {
         showToast(t('marketplace.messages.redeem_success'), 'success')
         ...
         showToast(res.error || t('dashboard_client.messages.payment_error'), 'error')
      }
   } catch(e) {
      const errorMessage = e.data?.error || e.message || t('dashboard_client.messages.payment_error')
...
const procesarPagoPaquete = async () => {
   try {
      ...
      if(res.success) {
         ...
         showToast(t('marketplace.messages.payment_sent'), 'success')
         await cargarPaquetesUsuario()
...
const usarPaquete = async (p) => {
   try {
      ...
      if(res.success) {
         showToast(t('marketplace.messages.package_activated'), 'success')
         await cargarPaquetesUsuario()
   ...
   } catch(e) { showToast(t('dashboard_client.messages.activation_error'), 'error') }
...
const sendWA = async (p, ref, type, extraId = null) => {
   try {
      ...
      if (type === 'pay') {
         ...
         msg = `*${t('marketplace.receipt_whatsapp.title')}*\n\n*ID:* ${formattedDate}-${idPaquete}\n*${t('marketplace.package')}:* ${p.nombre}\n*${t('dashboard_client.receipt_whatsapp.number')}:* ${ref}\n*${t('dashboard_client.transfer_amount')}:* L. ${formatNumber(p.costo)}\n\n${t('marketplace.receipt_whatsapp.footer')}`
      } else {
         msg = `*${t('marketplace.use_package_whatsapp.title')}*\n\n${t('marketplace.use_package_whatsapp.desc')}\n\n*ID:* ${formattedDate}-${ref}\n*${t('marketplace.package')}:* ${p.nombre}\n${t('marketplace.use_package_whatsapp.footer')}`
      }
...
const copyToClipboard = async (text) => {
   try {
      ...
      if (navigator.clipboard && window.isSecureContext) {
         ...
         showToast(t('marketplace.messages.copy_success'), 'success')
         return
      }
      ...
      showToast(t('marketplace.messages.copy_error'), 'error')
   }
}
