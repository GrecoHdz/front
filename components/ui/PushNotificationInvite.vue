      ...
      >
        ...
        <div class="w-full space-y-4 pt-2 relative z-10">
          <button 
            ...
            class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white py-5 rounded-3xl text-lg font-extrabold hover:translate-y-[-2px] hover:shadow-xl hover:shadow-emerald-500/20 transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-3 group disabled:opacity-75 disabled:cursor-not-allowed disabled:transform-none"
          >
            <svg v-if="isSubscribing" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <span>{{ $t('notifications.push_invite.cta') }}</span>
            <svg v-if="!isSubscribing" class="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
